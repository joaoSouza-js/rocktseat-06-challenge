...
describe('mark deliver return (e2e)', () => {
    ...
    test('[PATCH] /deliver/:id/return', async () => {
        ...
        const response = await agent.patch(`/deliver/${deliver.id.toString()}/return`).send().set({
        ...
        expect(deliverPersisted?.status).toEqual(DeliveryStatus.RETURNED)
        expect(deliverPersisted?.deliveryId).toBeFalsy()
